...
;		call	depack	
		...
		xor		a														;definimos inglés como idioma de inicio
		ld		(idioma),a
...
RUTINA_DE_MENU:
		...
		ld		hl,freeware												;mensaje de freeware
		ld		de,#0380
		ld		bc,8*8
		call	GRABAVRAM		
...
bloque_sanctuary_2_cont:
		...
		ld		(posicion_puerta),hl
		ld		a,1
		ld		(velocidad_paseo_fases),a								;para que, si pasa la fase, en el paseillo valla más lento y de tiempo a leer el texto
...
pant_carga_til:	incbin	"PANTALLA_DE_CARGA_TILES_cancer.til"
pant_carga_col:	incbin	"PANTALLA_DE_CARGA_TILES_cancer.col"
...
titulo_1:		db	224,"bkc  f  f bklc bc  f   f  f  baca a  l  a k  a kdc l   a  a be jkbe  a  k alme l k m   m  k dc  ldkc k  l lmkc m m a   l  a  dc m  l l  m m  a a dcm   k  l   dca  m dale g  g g  de   daleprtvalhme                       qsuwl"    
titulo_8:		db	61,"daalmakaklkamakamkakakamlakaalae   CONTRA  EL CANCER INFANTIL" 
mensaje:		db	4,"MENU"
...
freeware:		DB	$3D,$21,$39,$21,$21,$20,$20,$21						;DEFINICION DE FREEWARE
				DB 	$20,$20,$2F,$00,$00,$00,$00,$00
				DB 	$CF,$28,$CE,$28,$2F,$00,$C2,$22
				DB 	$2A,$2A,$94,$00,$00,$00,$00,$00
				
				DB 	$7F,$40,$70,$40,$78,$00,$F7,$94
				DB 	$F7,$94,$94,$00,$00,$00,$00,$00
				DB 	$FC,$00,$04,$04,$04,$00,$3C,$A0
				DB 	$38,$A0,$BC,$00,$00,$00,$00,$00		
...
idioma:							equ	inicio_ram+1						;0 ingles 1 castellano 2 catalán

petiso_que_toca:				equ	idioma+1							;0 quieto 1 saltando
petisox:						equ	petiso_que_toca+1
petisoy:						equ	petisox+1
espera_petiso:					equ	petisoy+1
espera_petiso_resta:			equ	espera_petiso+1
espera_petiso_resta_2:			equ	espera_petiso_resta+1

VIEJA_INTERR:					EQU	espera_petiso_resta_2+1				;le damos 5 bytes

estado_serp:					equ	VIEJA_INTERR+5						;ESTADO Y POSICION DE LA SERPIENTE DE SELECCION
y_serp:							equ	estado_serp+1

stick_a_usar:					equ	y_serp+1							;valor de on stick y on strig para el juego

clock:							equ	stick_a_usar+1						;variables de tiempo para intercalar menu y creditos

px:								equ	clock+2								;x prota
py:								equ	px+1								;y prota
puede_cambiar_de_direccion:		equ	py+1								;0 - no 1 - si
vidas_prota:					equ	puede_cambiar_de_direccion+1
color_prota:					equ	vidas_prota+1						;7 - mojado		8 - seco
color_lineas_prota:				equ	color_prota+1						;1 - seco		14 - moojado
muerto:							equ	color_lineas_prota+1				;0 muerto - 1 vivo
fase_en_la_que_esta:			equ	muerto+1							;fase en la que jugamos
estado_prota:					equ	fase_en_la_que_esta+1				;0=quieto 1=andando 2=saltando 3=cayendo 4=subiendo/bajando
prev_dir_prota:					equ	estado_prota+1						;hacia qué lado miraba antes el prota
pasos_de_salto:					equ	prev_dir_prota+1					;si está saltando indica cuánto le queda para dejar de saltar
dir_prota:						equ	pasos_de_salto+1					;0 derecha 1 izquierda
paso:							equ	dir_prota+1							;0 recto 1 andando 2 subiendo A 3 subiendo B
retard_anim:					equ	paso+1								;entre 0 y 9
atributos_sprite_general:		equ	retard_anim+1						;4 bytes y,x,patron,color PARA 4 SPRITES
tiene_objeto:					equ	atributos_sprite_general+48			;0=no 1=bomba 2=cuchillo
px_salida:						equ	tiene_objeto+1						;para recordar el punto de la puerta
py_salida:						equ	px_salida+1

x_explosion:					equ	py_salida+1
y_explosion:					equ	x_explosion+1
estado_de_explosion:			equ	y_explosion+1						;0=no 1,2,3 sprites 22,23,24
contador_retardo_explosion:		equ	estado_de_explosion+1

buffer_colisiones:				equ	contador_retardo_explosion+1		;para leer las colisiones en RAM ya que es mucho más ràpido

estado_de_salto:				equ	buffer_colisiones+736				;1a8-9a20-21a28 salto derecha - 31a38-39a50-51a58 salto izquierda
mechero:						equ	estado_de_salto+1					;0 no lo tienes 1 lo tienes
gasolina:						equ	mechero+1							;0 no lo tienes 1 lo tienes
posicion_puerta:				equ	gasolina+1							;posicion de la puerta
ya_ha_cambiado_puerta:			equ	posicion_puerta+2					;0 está cerrada 1 está abierta
increm:							equ	ya_ha_cambiado_puerta+1				;para borrado concreto de vram

score:							equ	increm+1							;puntos
contador_para_puntuacion:		equ	score+2								;compara con score para controlar los tiles a pintar en puntuacion
posicion_del_punto_unidades:	equ contador_para_puntuacion+2			;control tile unidades
posicion_del_punto_decenas:		equ	posicion_del_punto_unidades+1		;control tile decenas
posicion_del_punto_centenas:	equ	posicion_del_punto_decenas+1		;control tile centenas
posicion_del_punto_millares:	equ	posicion_del_punto_centenas+1		;control tile unidades de millar
cuenta_puntos_o_no:				equ	posicion_del_punto_millares+1		;0 no cuenta 1 si cuenta
cuanto_sumamos_a_score:			equ	cuenta_puntos_o_no+1				;para el contador evolutivo

variables_serpiente_1:			equ	cuanto_sumamos_a_score+1			;	0	-	serp_x_1
																		;	1	-	serp_y_1
																		;	2	-	serp_direcion_1
																		;	3	-	serp_paso_1
																		;	4	-	color_serp_1
...
variables_serpiente_2:			equ	variables_serpiente_1+9				;	0	-	serp_x_2
																		;	1	-	serp_y_2
																		;	2	-	serp_direcion_2
																		;	3	-	serp_paso_2
																		;	4	-	color_serp_2
...
variables_serpiente_3:			equ	variables_serpiente_2+9				;	0	-	serp_x_3
																		;	1	-	serp_y_3
																		;	2	-	serp_direcion_3
																		;	3	-	serp_paso_3
																		;	4	-	color_serp_3
...
variables_serpiente_4:			equ	variables_serpiente_3+9				;	0	-	serp_x_4
																		;	1	-	serp_y_4
																		;	2	-	serp_direcion_4
																		;	3	-	serp_paso_4
																		;	4	-	color_serp_4
...
colision_cuchillo_serp_real:	equ	variables_serpiente_4+9				;	0	-	no hay colision, 1	-	si la hay															
serp1:							equ	colision_cuchillo_serp_real+1		;serpiente 1 1-presente 0-ausente
serp2:							equ	serp1+1								;serpiente 2 1-presente 0-ausente
serp3:							equ	serp2+1								;serpiente 3 1-presente 0-ausente
serp4:							equ	serp3+1								;serpiente 4 1-presente 0-ausente

cont_no_salta_dos_seguidas:		equ	serp4+1							;para evitar el doble salto

pasamos_la_fase:				equ	cont_no_salta_dos_seguidas+1		;0 estamos jugando 1 pasamos la fase

momento_lanzamiento:			equ	pasamos_la_fase+1					;0 no lanza - 1 movm 1 - 2 mov 2 - 3 en órbita
contador_poses_lanzar:			equ	momento_lanzamiento+1				;para retrasar las poses de lanzar
recordar_lo_que_habia:			equ	contador_poses_lanzar+1				;guarda el valor de lo que borra si hay que pintar circunstancialmente una cosa
dir_cuchillo:					equ	recordar_lo_que_habia+2				;la dirección hacia la que ba el cuchillo
cx:								equ	dir_cuchillo+1						;x cuchillo
cy:								equ	cx+1								;y cuchillo

escalera_activada:				equ	cy+1								;0 no - 1 si
posicion_escalera:				equ	escalera_activada+1					;numero de tile que tiene que pintar de la escalera alternativa
contador_escalera:				equ	posicion_escalera+2					;retardo para la construccion de la escalaera
limite_escalera					equ	contador_escalera+1					;numero de tile en el que tiene que parar de construir


grifo_estado:					equ	limite_escalera+2					;0 - cerrado	1 - abierto


ESTADO_MUSICA:					EQU	grifo_estado+1						;0 - apagada y 1 - encendida

vision_estado:					equ	ESTADO_MUSICA+1						;	1 	-	ve nada y 1 - lo ve todo

colision_bombero_prota_real:	equ	vision_estado+1						;	0	-	no hay colision,	1	-	si la hay			
bombero_1:						equ	colision_bombero_prota_real+1		;	0 	- 	existe				1 	- 	no existe
bombero_control:				equ	bombero_1+1							;	0	-	x					(0 - 255)
																		;	1	-	y					(0 - 255)
																		;	2	.	velocidad			(1 - 3)
...
velocidad_paseo_fases:			equ	bombero_control+12					;1 - mas tiempo para leer mensaje 2-menos tiempo
SAXOLO:							EQU	velocidad_paseo_fases+1				;MUSICA INICIO
GAME_OVER:						EQU	SAXOLO+1841							;MUSICA FIN PARTIDA

MUTE:							EQU	GAME_OVER+758						;silencio musical								
MUERTO:							EQU	MUTE+117							;MUSICA MUERTO
ENTRE_FASES:					equ	MUERTO+292							;MUSICA ENTRE FASES
